...
const services = ref([
  ...
  "Group Coaching/Training Session",
  "Others",
])
...
<template>
  ...
  <Container>
    ...
    <section class="mt-[2rem] mb-[5rem]">
      <Form class="font-[500] text-[2rem] leading-[4.2rem] text-secondaryColor contact_form" @submit="sendEmail" :validation-schema="validationSchema">
        <div class="flex flex-row justify-between gap-y-[2.7rem] flex-wrap">
          ...
          <div class="w-[47.7rem]">
            ...
            <Field v-slot="{ value }" name="service" as="select" key="service">
              <option v-for="(item, index) in services" :key="index" :selected="value" :value="item">{{ item }}</option>
            </Field>
